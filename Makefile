# Version is set from git tags (e.g., v0.1.0), falling back to "dev"
VERSION ?= $(shell git describe --tags --always --dirty 2>/dev/null || echo "dev")

# Image URL to use all building/pushing image targets
IMG ?= baseten-operator:$(VERSION)
...
.PHONY: build
build: manifests generate fmt vet ## Build baseten-operator binary.
	go build -ldflags "-X main.version=$(VERSION)" -o bin/baseten-operator cmd/main.go
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name baseten-operator-builder
	$(CONTAINER_TOOL) buildx use baseten-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm baseten-operator-builder
	rm Dockerfile.cross
