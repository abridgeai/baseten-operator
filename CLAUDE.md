...
# Deploy operator to cluster (Helm installs CRDs + controller + RBAC)
make deploy IMG=<registry>/baseten-operator:tag

# Apply sample CR
kubectl apply -f config/samples/models_v1alpha1_basetenmodel.yaml

# Remove operator from cluster (Helm uninstalls everything)
make undeploy

# `make install` / `make uninstall` are an alternative path for CRD-only installs
# (via `kubectl apply -f config/crd/bases/`) for GitOps flows. Do not combine with
# `make deploy` — Helm refuses to adopt CRDs it did not create.
```
...
# Deploy operator (Helm installs CRDs + controller as one release)
...
charts/baseten-operator/   # Helm chart (user-facing install)
  templates/               # Rendered K8s resources
  files/basetenmodel-crd.yaml  # CRD (synced from config/crd/bases/ by `make manifests`)
  values.yaml
config/                    # Kubebuilder-generated source (internal; drives the chart)
  crd/bases/               # CRD regenerated by controller-gen
...
3. Run `make manifests generate` to update CRDs and generated code (also syncs the CRD into `charts/baseten-operator/files/`)
4. If running in a cluster, `make deploy` to reinstall with the updated CRD
...
- User-facing install via the Helm chart in `charts/baseten-operator/`
- `config/` holds only kubebuilder-generated CRD + RBAC source, which the Helm chart consumes (CRD synced via `make manifests`, RBAC rules manually mirrored in `templates/clusterrole-manager.yaml`)
